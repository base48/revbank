...
RevBank is a self-service checkout and payment system for hackerspaces, primarily intended for drinks and snacks.

This document describes the software architecture. For more hands-on documentation, refer to the other documents.

=head1 High-level architecture
...
Originally, the RevBank core did B<not> have any notion of "products". Support for buying products through RevBank is provided by plugins, like the included C<products> and C<market> plugins. It is easy to add another source of products by writing another plugin. The C<products> plugin uses the built-in products system (L<RevBank::Products>), but it could also be disabled. A plugin contains arbitrary code and can do anything, including querying external resources.
