...
One or more product ids, separated by semicolons. The first product id on the
line is considered canonical, the rest are aliases.

There may be whitespace around the semicolons, and product IDs may be quoted.

The following three examples are the same:

	primary_id;alias1;alias2       0.42  "Example"
	primary_id ; alias1 ; alias2   0.42  "Example"
	"primary_id"; alias1; alias2   0.42  "Example"
...
I<In older RevBank versions, the product IDs were separated by commas with no
whitespace surrounding them. This will continue to be supported until early
2028 but prints a warning. In the meantime, to have a primary ID with a literal
comma in it, you can prefix it with a semicolon to force the new parsing (the
empty product ID before the semicolon is ignored).>

It is technically possible to include a semicolon or whitespace in a product
ID, by quoting or escaping (e.g. C<\;>), but the user would also have to use
quotes or escapes to enter these, so it's not readily barcode scanner
compatible.
